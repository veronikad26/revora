...
from app.config import MAX_CUSTOMER_CONTACTS, OPT_OUT_KEYWORDS
...
def _reference(state: RecoveryState) -> str:
    return str(
        state.get("invoice_payment_ref")
        or state.get("cart_id")
        or state.get("payment_id")
        or state["case_id"]
    )
...
def build_message_prompt(
    ...
    *,
    ...
    correction = (
        " Previous draft failed safety review; obey every constraint exactly."
        if corrective
        else ""
    )
    ...
    return (
        ...
        "Use only the supplied reference and amount. Do not include URLs, payment links, "
        "OTP/CVV requests, threats, urgency, discounts, fee changes, or new dates. "
        "Do not claim payment was made. Tell the customer they can open their payment app "
        "and reply STOP to opt out. Do not mention internal systems, policies, confidence, "
        "or automation."
        + correction
    )
...
def _intent_dict(intent: CustomerIntent) -> dict[str, Any]:
    return {
        ...
        "payment_confirmed": intent.payment_confirmed,
    }
...
def _inbound_update(
    ...
    update: dict[str, Any] = {
        ...
        "consent_flag": (
            False if intent.opt_out else state.get("consent_flag", False)
        ),
        "contact_allowed": (
            False if intent.opt_out else state.get("contact_allowed", False)
        ),
        "dispute_flag": bool(intent.dispute_flag),
        "payment_confirmed": bool(intent.payment_confirmed),
    ...
    update.update(
        ...
        else on_inbound_reply(state, intent_dict)
    )
    ...
    *,
    ...
    if state.get("contact_count", 0) >= MAX_CUSTOMER_CONTACTS:
        return {
            ...
            "execution_result": "communication_blocked_contact_limit",
        }

    if state.get("opt_out"):
        return {
            "proposed_action": "do_nothing",
            "execution_result": "communication_blocked_opt_out",
        }

    if state.get("proposed_action") not in OUTBOUND_ACTIONS:
        return {
            "execution_result": "communication_not_required",
        }
